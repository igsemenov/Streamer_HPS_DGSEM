...
###############################################################################
def getMe(E):
    return (0.1202)/(E**0.26)

def getDe(E):
    return (5.49244e-3)*(E**0.22)

def getKi(E):

    Ki=(119.44+(4.3666e7)/(E*E*E))*np.exp(-273./E)

    Ki=(0.1202*(E**0.74))*(Ki-0.034075)
...
#------------------------------------------------------------------------------
    def step_fvs(self,dt,nt,fvs,lgn):

        for j in range(nt):

            for elm in self.elm:
                elm.psn.r[:,:]=fvs.f2g(elm.fld_fvs.f[0]-elm.fld_fvs.f[2])
            ...
            for elm in self.elm:
                ...
                fld.f[5][:,:]=-1809.*fvs.g2f(psn.f[2])-15.
            ...
            for elm in self.elm:
                ...
                fld.f[1]=fld.f[0]+dt*fld.df[0]+dt*fld.df[1]
            ...
            for elm in self.elm:
                ...
                fld.f[0]=0.5*(fld.f[0]+fld.f[1]+dt*fld.df[0]+dt*fld.df[1])
                fld.f[2]=0.5*(fld.f[2]+fld.f[3]+dt*fld.df[1])

        self.t=self.t+nt*dt

#------------------------------------------------------------------------------
    def step_dgs(self,dt,nt,dgs,fvs,lgn):
...
#------------------------------------------------------------------------------
    ...
    def charge(self,lgn):
        ...
        for elm in self.elm:
            ...
            rho1=(fld.f[2]-fld.f[0])*nod.x

            rho2=fld.f[0]*nod.x

            rho1=np.sum(lgn.I*rho1)*(fld.h[0]*fld.h[0])
            rho2=np.sum(lgn.I*rho2)*(fld.h[0]*fld.h[0])
